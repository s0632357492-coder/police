...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('voicekick')
    .setDescription('ดึงสมาชิกออกจากห้องเสียง (Moderator หลัก หรือ Special Moderator ของห้องนั้น)')
  ...
  async execute(interaction) {
    ...
    if (!specialVoiceService.canActOnTarget(interaction.member, target)) {
    ...
    await target.voice.disconnect(reason).catch((err) => { throw err; });

    await reasonService.recordAndCheckThreshold(interaction.client, interaction.guild, interaction.user.id, 'kick', target.id, reason);

    await interaction.editReply({ embeds: [successEmbed('ดึงออกจากห้องเสียงสำเร็จ', `<@${target.id}> ถูกดึงออกจากห้องเสียง\nเหตุผล: ${reason}`)] });
  },
};
